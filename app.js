...
app.get('/', async function (req, res) {
  try {
    const sql1 = "SELECT product_id, product_name, product_price, product_price_promotion, MAX(ImageSrc_table.Image_Path) AS Path FROM product_table JOIN ImageSrc_table ON product_table.product_imageID = ImageSrc_table.product_imageID WHERE product_price_promotion > 0 GROUP BY product_id, product_name, product_price, product_price_promotion ORDER BY product_price_promotion ASC LIMIT 8;";
    const sql2 = "SELECT product_table.product_id, product_name, product_price, product_price_promotion, product_description, MAX(ImageSrc_table.Image_Path) AS Path, number_of_items FROM product_table JOIN ImageSrc_table ON product_table.product_imageID = ImageSrc_table.product_imageID JOIN order_table ON product_table.product_id = order_table.product_id GROUP BY product_table.product_id, product_name, product_price, product_price_promotion, product_description, number_of_items ORDER BY number_of_items DESC LIMIT 8;";
    const sql3 = "SELECT product_id, product_name, product_price, product_price_promotion, MAX(ImageSrc_table.Image_Path) AS Path FROM product_table JOIN ImageSrc_table ON product_table.product_imageID = ImageSrc_table.product_imageID GROUP BY product_id, product_name, product_price, product_price_promotion ORDER BY product_id DESC LIMIT 8;";

    db.queryCallback(sql1, function (err, flase) {
      if (err) throw err;

      db.queryCallback(sql2, function (err, bsale) {
        if (err) throw err;
